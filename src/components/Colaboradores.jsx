...
export const Colaboradores = ({ selectedSeller, setSelectedSeller, isVendedor, globalUser, salesData, goalsDB = {}, usersDB = {}, setAuthModal }) => {
    ...
    return (
        <div className="flex flex-col min-h-full animate-fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4 bg-white p-4 rounded-2xl border border-neutral-200 shadow-sm shrink-0">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-red-50 flex items-center justify-center text-[#E3000F]">
                        <Users size={22} />
                    </div>
                    <div>
                        <h2 className="text-lg font-bold text-neutral-800">Desempenho da Equipe</h2>
                        <p className="text-xs text-neutral-500 font-medium">Acompanhe o atingimento de metas mês a mês.</p>
                    </div>
                </div>
                <div className="flex items-center gap-3 bg-neutral-50 p-2 rounded-xl border border-neutral-200 w-full sm:w-auto">
                    <Calendar size={18} className="text-neutral-500 ml-2" />
                    <input 
                        type="month" 
                        value={monthFilter} 
                        onChange={(e) => setMonthFilter(e.target.value)}
                        className="bg-transparent text-sm font-bold text-neutral-700 outline-none pr-2 cursor-pointer focus:text-[#E3000F] w-full sm:w-auto"
                    />
                </div>
            </div>
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {safeVendedores.map(seller => {
                            ...
                            return (
                                <div key={seller} onClick={() => handleSellerClick(seller)} className="bg-white rounded-2xl p-6 border border-neutral-200 shadow-sm hover:shadow-lg hover:border-[#E3000F]/30 transition-all cursor-pointer group relative overflow-hidden">
                                    {isRestricted && (
                                        <div className="absolute top-4 right-4 text-neutral-300 group-hover:text-[#E3000F] transition-colors" title="Acesso Sigiloso">
                                            <Lock size={16} />
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-3 pt-4 border-t border-neutral-100">
                                        {isRestricted ? (
                                            <div className="flex flex-col items-center justify-center py-2 opacity-50">
                                                <Lock size={20} className="mb-1 text-neutral-400" />
                                                <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Acesso Sigiloso</span>
                                            </div>
                                        ) : (
                                            <>
                ...
                <div className="flex flex-col h-full animate-fade-in">
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm gap-4 shrink-0">
                        ...
                        <div className="md:min-w-[300px]">
                            <ProgressBar label="Receita Total Acumulada" realizado={getSellerMetrics(selectedSeller).totalReceita} meta={individualMetas.receita} isCurrency={true} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 pb-6 pr-2">
                        <div className="bg-white rounded-3xl p-6 border border-neutral-200 shadow-sm lg:col-span-2 flex flex-col relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="font-bold text-neutral-800 text-lg uppercase tracking-wide">GROSS TOTAL</h3>
                            </div>
                            <div className="mb-8">
                        ...
                        <div className="bg-neutral-900 rounded-3xl p-6 shadow-sm text-white flex flex-col relative overflow-hidden">
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-auto pt-6 border-t border-neutral-700">
                                <div className="flex flex-col justify-center bg-neutral-800/80 p-4 rounded-2xl border border-neutral-700/50 hover:bg-neutral-800 transition-colors">
                                    ...
                                    <div className="flex items-baseline gap-1"><span className="text-3xl font-black text-white leading-none">{getSellerMetrics(selectedSeller).volFibra}</span><span className="text-[10px] font-medium text-neutral-500 mb-1">/ {individualMetas.fibra}</span></div>
                                </div>
                                <div className="flex flex-col justify-center bg-neutral-800/80 p-4 rounded-2xl border border-neutral-700/50 hover:bg-neutral-800 transition-colors">
                                    ...
                                    <div className="flex items-baseline gap-1"><span className="text-3xl font-black text-white leading-none">{getSellerMetrics(selectedSeller).volTv}</span><span className="text-[10px] font-medium text-neutral-500 mb-1">/ {individualMetas.tv}</span></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 border border-neutral-200 shadow-sm flex flex-col">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="font-bold text-neutral-800 text-sm uppercase tracking-wide">METAS APARELHOS & ACESSORIOS</h3>
                            </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 border border-neutral-200 shadow-sm lg:col-span-2 flex flex-col">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="font-bold text-neutral-800 text-sm uppercase tracking-wide">METAS SERVIÇOS ADICIONAIS</h3>
                            </div>
