...
export function Proposta({ globalUser }) {
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 min-h-full animate-fade-in print:block print:h-auto print:overflow-visible">
            ...
            <div className="w-full lg:w-[35%] flex flex-col gap-4 no-print pr-2 pb-8">
                <div className="bg-white rounded-2xl shadow-sm border border-neutral-200 p-5 shrink-0">
                    <div className="flex items-center gap-3 mb-5">
                        <div className="w-10 h-10 rounded-xl bg-red-50 flex items-center justify-center text-[#E3000F]">
                            <Calculator size={22} />
                        </div>
                        <div>
                            <h2 className="text-base font-bold text-neutral-800">Simulador de Proposta</h2>
                            <p className="text-xs text-neutral-500 font-medium">Monte o combo e visualize os descontos.</p>
                        </div>

                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider flex items-center gap-2"><Smartphone size={14} className="text-[#E3000F]" /> Aparelho Celular e Valor (R$)</label>
                            <div className="flex gap-2">
                                <input type="text" value={aparelhoNome} onChange={(e) => setAparelhoNome(e.target.value)} placeholder="Modelo do aparelho" className="w-2/3 bg-neutral-50 border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium uppercase" />
                                <input type="text" value={aparelhoValor} onChange={(e) => setAparelhoValor(applyCurrencyMask(e.target.value))} placeholder="R$ 0,00" className="w-1/3 bg-neutral-50 border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium" />
                            </div>
                        </div>

                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider flex items-center gap-2"><ShieldCheck size={14} className="text-[#E3000F]" /> Seguro Proteção Móvel</label>
                            <select value={seguro} onChange={(e) => setSeguro(e.target.value)} className="w-full bg-white border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium cursor-pointer">
                                <option value="">Não ofertar seguro</option>
                                {seguroPlans.map(p => <option key={p.label} value={p.label}>{p.label}</option>)}
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            ...
                            <input type="text" value={cliente} onChange={(e) => setCliente(e.target.value)} placeholder="Ex: João da Silva" className="w-full bg-neutral-50 border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium" />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={movel} onChange={(e) => setMovel(e.target.value)} className="w-full bg-white border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium cursor-pointer">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={fibra} onChange={(e) => setFibra(e.target.value)} className="w-full bg-white border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium cursor-pointer">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={tv} onChange={(e) => setTv(e.target.value)} className="w-full bg-white border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium cursor-pointer">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={fixo} onChange={(e) => setFixo(e.target.value)} className="w-full bg-white border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium cursor-pointer">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={mesh} onChange={(e) => setMesh(e.target.value)} className="w-full bg-white border border-neutral-200 text-neutral-800 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-[#E3000F] outline-none text-sm font-medium cursor-pointer">
            ...
            <div className="w-full lg:w-[65%] flex flex-col print:w-full print:block print:h-auto print:overflow-visible">
                <div className="w-full flex flex-col sm:flex-row gap-3 no-print mb-4 justify-end">
                    <button onClick={handleDownloadImage} disabled={valorTotal === 0 && valAparelho === 0} className="px-6 py-2.5 bg-neutral-900 text-white font-bold rounded-xl hover:bg-black transition-colors shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm">
                ...
                <div id="printable-proposal" className="print-proposta bg-white w-full rounded-2xl shadow-xl border border-neutral-200 overflow-hidden print:overflow-visible relative print:shadow-none print:border print:border-neutral-300 print:w-full print:block print:max-w-[190mm] print:mx-auto" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                    
                    {/* Cabeçalho da Proposta */}
                    <div className="bg-neutral-900 text-white p-5 md:p-6 relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 bg-white print:grid-cols-12">
                        
                        {/* Coluna 1: Cliente e Resumo de Serviços (Lado Esquerdo) */}
                        <div className="md:col-span-7 lg:col-span-7 print:col-span-7 p-6 print:p-5 flex flex-col border-b md:border-b-0 md:border-r border-neutral-100 print:border-r print:border-neutral-200">
                            <div className="mb-6 pb-5 border-b border-neutral-100 border-dashed">
                                <div className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-0.5">Proposta para</div>
                                <div className="text-lg font-bold text-neutral-800 leading-tight">{cliente || 'Cliente Não Informado'}</div>
                            </div>

                            <div className="space-y-3.5 mb-6 flex-1">
                                <div className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">Resumo dos Serviços</div>
                                
                                {movel && (
                                    <div className="flex justify-between items-center group">
                                        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-neutral-50 flex items-center justify-center text-neutral-600 border border-neutral-100"><Smartphone size={14} /></div><span className="text-xs font-bold text-neutral-700 leading-tight w-36 sm:w-auto truncate">{movel}</span></div>
                                        <span className="text-xs font-black text-neutral-900">{applyCurrencyMask(valMovel)}</span>
                                    </div>
                                )}
                                {fibra && (
                                    <div className="flex justify-between items-center group">
                                        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-neutral-50 flex items-center justify-center text-neutral-600 border border-neutral-100"><Wifi size={14} /></div><span className="text-xs font-bold text-neutral-700 leading-tight w-36 sm:w-auto truncate">Fibra {fibra}</span></div>
                                        <span className="text-xs font-black text-neutral-900">{applyCurrencyMask(valFibra)}</span>
                                    </div>
                                )}
                                {tv && (
                                    <div className="flex justify-between items-center group">
                                        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-neutral-50 flex items-center justify-center text-neutral-600 border border-neutral-100"><Tv size={14} /></div><span className="text-xs font-bold text-neutral-700 leading-tight w-36 sm:w-auto truncate">{tv}</span></div>
                                        <span className="text-xs font-black text-neutral-900">{applyCurrencyMask(valTv)}</span>
                                    </div>
                                )}
                                {fixo && (
                                    <div className="flex justify-between items-center group">
                                        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-neutral-50 flex items-center justify-center text-neutral-600 border border-neutral-100"><Phone size={14} /></div><span className="text-xs font-bold text-neutral-700 leading-tight w-36 sm:w-auto truncate">{fixo}</span></div>
                                        <span className="text-xs font-black text-neutral-900">{applyCurrencyMask(valFixo)}</span>
                                    </div>
                                )}
                                {mesh && (
                                    <div className="flex justify-between items-center group">
                                        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-neutral-50 flex items-center justify-center text-neutral-600 border border-neutral-100"><Router size={14} /></div><span className="text-xs font-bold text-neutral-700 leading-tight w-36 sm:w-auto truncate">{mesh}</span></div>
                                        <span className="text-xs font-black text-neutral-900">{applyCurrencyMask(valMesh)}</span>
                                    </div>
                                )}
                                {seguro && (
                                    <div className="flex justify-between items-center group">
                                        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-neutral-50 flex items-center justify-center text-neutral-600 border border-neutral-100"><ShieldCheck size={14} /></div><span className="text-xs font-bold text-neutral-700 leading-tight w-36 sm:w-auto truncate">{seguro} <span className="text-[9px] font-normal text-neutral-500 block">(Boleto no Plano Móvel)</span></span></div>
                                        <span className="text-xs font-black text-neutral-900">{applyCurrencyMask(valSeguro)}</span>
                                    </div>
                                )}
                                {valorTotal === 0 && (
                                    <div className="text-center py-4 text-neutral-400 text-xs font-medium border border-dashed border-neutral-200 rounded-lg bg-neutral-50/50">
                            ...
                            <div className="bg-neutral-50 rounded-xl p-4 border border-neutral-200">
                                {comboType !== 'SINGLE' && valorTotal > 0 && (
                                    <div className="flex justify-between items-center mb-2 pb-2 border-b border-neutral-200/60">
                                        <span className="text-[10px] font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1"><CheckCircle2 size={10} /> Benefício {comboType} Aplicado</span>
                                        <span className="text-[10px] font-bold text-neutral-400 line-through">De: {applyCurrencyMask(valorSemCombo)}</span>
                                    </div>
                                )}
                                <div className="flex justify-between items-end">
                                    <div>
                                        <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest block mb-0.5">Plano Mensal</span>
                            ...
                            {valAparelho > 0 && (
                                <div className="mt-4 pt-4 border-t border-neutral-200 border-dashed">
                                    <div className="mb-3">
                                        <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest block mb-0.5">Oferta Exclusiva de Aparelho</span>
                                        <h4 className="text-sm font-black text-neutral-800 uppercase">{aparelhoNome || 'Aparelho Celular'}</h4>
                                    </div>
                                    
                                    <div className="grid grid-cols-2 gap-2 mb-2">
                                        <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-200 flex flex-col items-center text-center justify-center">
                                            <span className="text-[10px] font-bold text-neutral-500 uppercase">Valor À Vista</span>
                                            <span className="text-sm font-black text-neutral-900">{applyCurrencyMask(valAparelho)}</span>
                                        </div>
                                        <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-200 flex flex-col items-center text-center justify-center">
                                            <span className="text-[10px] font-bold text-neutral-500 uppercase">12x Sem Juros</span>
                                            <span className="text-sm font-black text-neutral-900">12x de {applyCurrencyMask(parcela12x)}</span>
                                        </div>
                                    </div>

                                    <div className="flex justify-between items-center bg-red-50 p-3 rounded-lg border border-red-100 mt-2">
                                        <div className="flex flex-col pr-2">
                                            <span className="text-xs font-bold text-red-800 uppercase tracking-wide">21x Sem Juros</span>
                                            <span className="text-[9px] text-red-600 mt-0.5 leading-tight max-w-[200px]">Exclusivo para Multi nos cartões: Bradesco, Santander, Itaú, Caixa, C6 Bank, PicPay, BB e Original.</span>
                                        </div>
                        ...
                        <div className="md:col-span-5 lg:col-span-5 print:col-span-5 bg-green-50/20 print:bg-green-50/40 p-6 print:p-5 flex flex-col">
                            <h3 className="text-[11px] font-bold text-green-800 uppercase tracking-widest mb-5 flex items-center gap-2">
                                <ShieldCheck size={16} className="text-green-600" /> Benefícios Inclusos
                            </h3>
                            
                            <div className="space-y-4 flex-1">
                                {beneficios.length === 0 ? (
                                    <p className="text-xs text-neutral-400 text-center mt-10">Adicione produtos para ver as vantagens.</p>
                                ) : (
                                    beneficios.map((b, idx) => (
                                        <div key={idx} className="flex gap-3 items-start">
                                            <div className="mt-0.5 text-green-600 bg-green-100 p-1.5 rounded-lg shrink-0">{b.icon}</div>
                                            <div>
                                                <h4 className="text-xs font-bold text-neutral-800 leading-tight mb-0.5">{b.title}</h4>
                                                <p className="text-[10px] text-neutral-500 leading-snug">{b.desc}</p>
                            ...
                            <div className="mt-8 text-center pt-4 border-t border-neutral-200 hidden print:block">
                                <div className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest mb-1">Consultor de Vendas</div>
                                <div className="text-xs font-bold text-neutral-800">{globalUser?.name ? globalUser.name.split(' ')[0] : 'Consultor Claro'}</div>
                                <div className="text-[10px] text-neutral-500">Shopping União Osasco - Claro</div>
                                <div className="text-[09px] text-neutral-500">Email: {globalUser?.email || 'consultor@claro.com.br'}</div>
                                <div className="text-[08px] text-neutral-500">Av. dos Autonomistas, 1400 - Arco 169 - Vila Yara, Osasco - SP<br />06020-010</div>
                            </div>
                        </div>
                    </div>
                </div>
