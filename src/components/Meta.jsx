    ...
    return (
        <div className="h-full flex flex-col animate-fade-in">
            {!hasAccess ? (
                ...
                <div className="flex flex-col h-full bg-white rounded-3xl shadow-sm border border-neutral-200 overflow-hidden">
                    <div className="flex border-b border-neutral-200 bg-neutral-50 shrink-0">
                        <button onClick={() => setMetaActiveSubTab('DEFINIR')} className={`px-8 py-4 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 ${metaActiveSubTab === 'DEFINIR' ? 'border-[#E3000F] text-[#E3000F] bg-white' : 'border-transparent text-neutral-500 hover:text-neutral-800'}`}>Definir Metas</button>
                        <button onClick={() => setMetaActiveSubTab('COMPARATIVO')} className={`px-8 py-4 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 ${metaActiveSubTab === 'COMPARATIVO' ? 'border-[#E3000F] text-[#E3000F] bg-white' : 'border-transparent text-neutral-500 hover:text-neutral-800'}`}>Histórico Mês a Mês</button>
                    </div>
                        ...
                        <div className="flex flex-col flex-1 relative overflow-hidden">
                            ...
                            <div className="p-6 md:p-8 border-b border-neutral-100 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-neutral-50/50 shrink-0">
                                ...
                                <div className="flex items-center gap-4 bg-white p-2 rounded-xl shadow-sm border border-neutral-200">
                                    ...
                                    <input type="month" value={selectedGoalMonth} onChange={(e) => setSelectedGoalMonth(e.target.value)} className="bg-neutral-50 border border-neutral-200 text-[#E3000F] px-4 py-2 rounded-lg font-bold outline-none cursor-pointer" />
                                    <button type="button" onClick={handleCopyFromPreviousMonth} className="px-3 py-2 text-xs font-bold text-neutral-600 hover:text-[#E3000F] hover:bg-red-50 rounded-lg">Puxar Mês Anterior</button>
                                </div>
                            </div>
                                ...
                                <form onSubmit={saveGoals} className="max-w-6xl mx-auto space-y-6 pb-12">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                                        <div className="bg-white p-6 rounded-3xl border border-neutral-200 shadow-sm flex flex-col">
                                            <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-red-50 text-[#E3000F] rounded-lg"><Smartphone size={20} /></div><h4 className="font-bold text-neutral-800 text-sm uppercase tracking-wide">Móvel</h4></div>
                                            <div className="space-y-4"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Pós Total</label><input type="text" name="posTotal" value={goalForm.posTotal} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-4 py-2.5 rounded-xl text-lg font-black outline-none focus:ring-1 focus:ring-[#E3000F]" /></div><div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Pós-Pago</label><input type="text" name="posPago" value={goalForm.posPago} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Controle</label><input type="text" name="controle" value={goalForm.controle} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div></div></div>
                                        </div>
                                        <div className="bg-white p-6 rounded-3xl border border-neutral-200 shadow-sm flex flex-col">
                                            <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-neutral-800 text-white rounded-lg"><Home size={20} /></div><h4 className="font-bold text-neutral-800 text-sm uppercase tracking-wide">Residencial</h4></div>
                                            <div className="space-y-4"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">UR Total</label><input type="text" name="urTotal" value={goalForm.urTotal} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-4 py-2.5 rounded-xl text-lg font-black outline-none focus:ring-1 focus:ring-neutral-800" /></div><div className="grid grid-cols-3 gap-3"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Fibra</label><input type="text" name="fibra" value={goalForm.fibra} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">TV</label><input type="text" name="tv" value={goalForm.tv} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Fixo</label><input type="text" name="fixo" value={goalForm.fixo} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div></div></div>
                                        </div>
                                        <div className="bg-white p-6 rounded-3xl border border-neutral-200 shadow-sm flex flex-col">
                                            <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-orange-50 text-orange-500 rounded-lg"><Watch size={20} /></div><h4 className="font-bold text-neutral-800 text-sm uppercase tracking-wide">Hardware</h4></div>
                                            <div className="space-y-4"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Aparelhos</label><input type="text" name="aparelho" value={goalForm.aparelho} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-4 py-2.5 rounded-xl text-lg font-black outline-none focus:ring-1 focus:ring-orange-500" /></div><div className="grid grid-cols-2 gap-4"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Acessórios</label><input type="text" name="acessorio" value={goalForm.acessorio} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Películas</label><input type="text" name="pelicula" value={goalForm.pelicula} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div></div></div>
                                        </div>
                                        <div className="bg-white p-6 rounded-3xl border border-neutral-200 shadow-sm flex flex-col">
                                            <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-blue-50 text-blue-500 rounded-lg"><ShieldCheck size={20} /></div><h4 className="font-bold text-neutral-800 text-sm uppercase tracking-wide">Serviços / Adicionais</h4></div>
                                            <div className="grid grid-cols-2 gap-x-4 gap-y-5"><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Seguro</label><input type="text" name="seguro" value={goalForm.seguro} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">M-Play</label><input type="text" name="mplay" value={goalForm.mplay} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">Trocafy</label><input type="text" name="trocafy" value={goalForm.trocafy} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-neutral-500 uppercase">MESH</label><input type="text" name="mesh" value={goalForm.mesh} onChange={handleGoalChange} className="w-full bg-neutral-50 border border-neutral-200 px-3 py-2 rounded-xl text-sm font-bold outline-none" /></div></div>
                                        </div>
                                    </div>
                    ...
                    {metaActiveSubTab === 'COMPARATIVO' && (
                        <div className="flex-1 overflow-auto p-6 md:p-8 bg-neutral-50/50">
                            <div className="max-w-7xl mx-auto">
                                <div className="mb-6"><h2 className="text-2xl font-bold text-neutral-800 flex items-center gap-2"><LineChart className="text-[#E3000F]" /> Histórico Comparativo (Mês x Mês)</h2></div>
                                <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-x-auto">
                                    <table className="w-full text-sm text-left whitespace-nowrap">
                                        <thead className="bg-neutral-800 text-white uppercase text-[10px] tracking-wider">
                                        ...
                                        <tbody className="divide-y divide-neutral-100">
                                            {monthNames.map(month => {
                                                ...
                                                return (
                                                    <tr key={month} className={`hover:bg-neutral-50 transition-colors ${isCurrent ? 'bg-red-50/30' : ''}`}>
                                                        <td className={`px-6 py-4 font-bold tracking-wider sticky left-0 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.05)] ${isCurrent ? 'bg-red-50 text-[#E3000F]' : 'bg-white text-neutral-800'}`}>{month} {isCurrent && <span className="ml-2 text-[9px] bg-[#E3000F] text-white px-2 py-0.5 rounded-full">Atual</span>}</td>
                                                        <td className="px-6 py-4 font-black text-neutral-800">{applyCurrencyMask(m.receita)}</td><td className="px-6 py-4 font-bold text-neutral-800">{m.posTotal}</td><td className="px-6 py-4 text-neutral-500">{m.posPago}</td><td className="px-6 py-4 text-neutral-500">{m.controle}</td><td className="px-6 py-4 font-bold text-neutral-800">{m.urTotal}</td><td className="px-6 py-4 text-neutral-500">{m.fibra}</td><td className="px-6 py-4 text-neutral-500">{m.tv}</td><td className="px-6 py-4 text-neutral-500">{m.fixo || 0}</td><td className="px-6 py-4 font-medium text-orange-600">{m.aparelho}</td><td className="px-6 py-4 text-neutral-600">{m.acessorio}</td><td className="px-6 py-4 text-neutral-600">{m.pelicula}</td><td className="px-6 py-4 font-medium text-blue-600">{m.seguro}</td><td className="px-6 py-4 text-neutral-600">{m.mplay}</td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
