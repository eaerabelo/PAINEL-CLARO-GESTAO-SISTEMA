...
import { Cpu, Plus, FileText, Trash2, ExternalLink, X, BookOpen, Layers, Tag, Archive } from 'lucide-react';
...
export function Geek({ geekDocs = [], setGeekDocs, isGerente, globalUser }) {
    ...
    const canEdit = globalUser?.role === 'GEEK';
    ...
    const handleDelete = (id) => {
        if (!canEdit) {
            toast.error('Acesso negado. Apenas o perfil GEEK pode excluir documentos.');
            return;
        }
    ...
    return (
        <div className="h-full flex flex-col bg-neutral-50/50 dark:bg-neutral-950/50 rounded-2xl overflow-y-auto animate-fade-in transition-colors">
            <div className="p-6 pb-4 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center shrink-0">
                ...
                {canEdit && (
                    <button onClick={() => setIsModalOpen(true)} className="px-4 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2 hover:-translate-y-0.5">
                ...
                <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm p-4 flex items-center justify-center no-print">
                    <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl w-full max-w-md animate-fade-in flex flex-col transition-colors">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="space-y-1.5"><label className="text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Categoria <span className="text-[#E3000F]">*</span></label><select value={formData.categoria} onChange={e => setFormData({ ...formData, categoria: e.target.value })} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-neutral-800 dark:text-neutral-100 px-3 py-2.5 rounded-lg focus:ring-1 focus:ring-indigo-500 outline-none text-sm font-bold">{categorias.map(c => <option className="bg-white dark:bg-neutral-900 text-neutral-800 dark:text-neutral-100" key={c.id} value={c.id}>{c.id}</option>)}</select></div>
